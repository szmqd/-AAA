    ...
    <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="管理员头像" prop="avatar">
        <template slot-scope="scope">
          <!-- {{'http://124.223.98.197:9093/wx'+scope.row.avatar.split("/wx")[1]}} -->
          <img v-if="scope.row.avatar" :src="scope.row.avatar.replace('http://localhost:8080',$store.state.user.url)" width="40">
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    uploadAvatar: function(response) {
      this.dataForm.avatar = response.data.url.replace('http://localhost:8080',this.$store.state.user.url)
    },
    handleCreate() {
      this.resetForm()
    ...
    handleUpdate(row) {
      ...
      row.avatar=row.avatar.replace("http://localhost:8080",this.$store.state.user.url)
